...
class StatesWorker(threading.Thread):
    ...
    def __send(self, data):
        ...
        try:
            ...
            self.__manager.__send_json(data)
        except:
            utils.log("ERROR", "Can't send data.",'__send',self)
            if self.__run:
    ...
    def __exec_salt(self, id, module, parameter):
        ...
        utils.log("INFO", "State ID '%s' from module '%s' done, result '%s'."%(id,module,),'__exec_salt',self)
    ...
    def run(self):
        ...
        while self.__run:
            ...
            utils.log("INFO", "Running state '%s', #%s"%(state['stateid'], STATUS),'run',self)
            try:
                if state.get('module') in self.__builtins:
                    ...
                    (result,err_log,out_log) = self.__exec_salt(state)
