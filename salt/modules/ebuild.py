...
def install(name=None,
    ...
    old = list_pkgs()
    call = dict(('state_%s' % k,v) for k,v in __salt__['cmd.run_all'](cmd, output_loglevel='debug').iteritems() if k in ['stdout', 'stderr'])
    __context__.pop('pkg.list_pkgs', None)
    if call['retcode'] != 0:
        return _process_emerge_err(call['state_stdout'], call['state_stderr'])
    new = list_pkgs()
...
def update(pkg, slot=None, fromrepo=None, refresh=False):
    ...
    new = list_pkgs()
    return (salt.utils.compare_dicts(old, new), call)
...
def purge(name=None, slot=None, fromrepo=None, pkgs=None, **kwargs):
    ...
    ret[0].update(depclean(name=name, slot=slot, fromrepo=fromrepo, pkgs=pkgs))
    return ret
